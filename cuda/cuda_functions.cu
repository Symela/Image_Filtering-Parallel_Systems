  ...
  // if (0 < x && x < height-1 && 0 < y && y < (3*width)-3) { // to eswteriko thw eikonas
    for (i = x-1, k = 0 ; i <= x+1 ; i++, k++) {
      for (j = (y*3)-3, l = 0 ; j <= (y*3)+3 ; j += 3, l++) {
        if ( i < 0 || i > height-1 || j < 0 || j > (3*width)-3 ) {
          new_val_red += start[(width*3) * x + (y*3)] * filter[k][l] / 16.0;
          new_val_green += start[(width*3) * x + (y*3) + 1] * filter[k][l] / 16.0;
          new_val_blue += start[(width*3) * x + (y*3) + 2] * filter[k][l] / 16.0;
...
__global__ void kernel_grey_equal(uint8_t* start, uint8_t* end, int height, int width)
{
  size_t x = blockIdx.x * blockDim.x + threadIdx.x;
  size_t y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void kernel_rgb_equal(uint8_t* start, uint8_t* end, int height, int width)
{
  size_t x = blockIdx.x * blockDim.x + threadIdx.x;
  size_t y = blockIdx.y * blockDim.y + threadIdx.y;
